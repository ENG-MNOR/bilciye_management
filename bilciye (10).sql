...
--
-- Host: 127.0.0.1
-- Generation Time: May 01, 2024 at 08:18 AM
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `recycling_proc` (IN `ID` INT, IN `status_sp` VARCHAR(40), IN `oper` VARCHAR(40))   begin
case 
when oper="insert" then 
if exists (select * from recycling where waste_id=waste_id_sp and ind_id=ind_id_sp and cit_id=cit_id_sp and qty=qty_sp and recy_date=recy_date_sp and status=status_sp and date=date_sp) then 
select 'This info already exist' as msg;
else 
insert into recycling values(null,waste_id_sp,ind_id_sp,cit_id_sp,qty_sp,0.05,recy_date_sp,status_sp,date_Sp,0);
select 'Inserted success' as msg;
end if;

when oper="update" then 
if exists (select * from recycling where recy_id=ID) then
update recycling set status=status_sp where recy_id=ID;
select 'Updated success' as msg;
else 
select concat('This ID (',ID,') is not exist') as msg;
end if;
...
select recy_id ID, waste.waste_type 'Waste Type', industry.ind_name 'Industry Name', citizen.name 'Citizen Name', qty 'Quantity', rate 'Rate', recy_date 'Requested Date', status 'Status', date from recycling,waste,citizen,industry where recycling.waste_id=waste.waste_id and recycling.cit_id=citizen.cit_id and recycling.ind_id=industry.ind_id and recycling.deleted=0 and industry.ind_id=num;
end$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `View_recycling_admin` ()   begin
select recy_id ID, waste.waste_type 'Waste Type', industry.ind_name 'Industry Name', citizen.name 'Citizen Name', qty 'Quantity', rate 'Rate', recy_date 'Date', status 'Status', date from recycling,waste,citizen,industry where recycling.waste_id=waste.waste_id and recycling.cit_id=citizen.cit_id and recycling.ind_id=industry.ind_id and recycling.deleted=0;
end$$
...
(116, 'moha', '555', '123', 'moha@gmail.com', '9090', 2, '1344495110.jpeg', 'suuqa weyn', 0);
...
INSERT INTO `recycling` (`recy_id`, `waste_id`, `ind_id`, `cit_id`, `qty`, `rate`, `recy_date`, `status`, `date`, `deleted`) VALUES
(7, 2, 2, 7, 20, 0.05, '2024-04-05', 'Pending', '2024-04-30 18:25:45', 0),
(18, 2, 1, 7, 1, 0.05, '2024-04-30', 'Pending', '2024-04-30 18:25:45', 0),
(19, 2, 1, 116, 1, 0.05, '2024-04-30', 'done', '2024-04-30 18:25:45', 0);
...
(33, 116, '1669615400.jpeg', 'hhh', 'done', '2024-04-29 09:06:44', 0);
...
--
ALTER TABLE `recycling`
  MODIFY `recy_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=20;
