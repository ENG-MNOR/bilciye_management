...
--
-- Host: 127.0.0.1
-- Generation Time: Apr 27, 2024 at 09:45 PM
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `getDesc` (IN `num` INT)   begin
select rep_id, description from reporting where rep_id=num;
end$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `goverment_recyle_proc` (IN `ID` INT, IN `ind_id_sp` INT, IN `waste_id_sp` INT, IN `qty_sp` INT, IN `user_id_sp` INT, IN `oper` VARCHAR(40))   begin
case 
when oper="insert" then 
if exists (select * from goverment_recyle where ind_id=ind_id_sp and waste_id=waste_id_sp and qty=qty_sp) then 
select 'This info already exist' as msg;
else 
insert into goverment_recyle values(null,ind_id_sp,waste_id_sp,qty_sp,0.08,user_id_sp,0);
end if;
select 'Inserted success' as msg;

when oper="update" then 
if exists(select * from goverment_recyle where gov_id=ID) then 
update goverment_recyle set ind_id=ind_id_sp, waste_id=waste_id_sp, qty=qty_sp where gov_id=ID;
select 'Updated success' as msg;
else 
select concat('This ID (',ID,') is not exist') as msg;
end if;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `recycling_proc` (IN `ID` INT, IN `waste_id_sp` INT, IN `ind_id_sp` INT(40), IN `cit_id_sp` INT, IN `qty_sp` INT, IN `recy_date_sp` DATE, IN `user_id_sp` INT, IN `oper` VARCHAR(40))   begin
case 
when oper="insert" then 
if exists (select * from recycling where waste_id=waste_id_sp and ind_id=ind_id_sp and cit_id=cit_id_sp and qty=qty_sp and recy_date=recy_date_sp) then 
select 'This info already exist' as msg;
else 
insert into recycling values(null,waste_id_sp,ind_id_sp,cit_id_sp,qty_sp,0.05,recy_date_sp,user_id_sp,0);
select 'Inserted success' as msg;
end if;

when oper="update" then 
if exists (select * from recycling where recy_id=ID) then
update recycling set waste_id=waste_id_sp, ind_id=ind_id_sp, cit_id=cit_id_sp, qty=qty_sp, recy_date=recy_date_sp where recy_id=ID;
select 'Updated success' as msg;
else 
select concat('This ID (',ID,') is not exist') as msg;
end if;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `reporting_proc` (IN `ID` INT, IN `Status_sp` VARCHAR(40), IN `oper` VARCHAR(40))   begin
case 
when oper="update" then 
if exists (select * from reporting where rep_id=ID) then
update reporting set Status=Status_sp where rep_id=ID;
select 'Updated success' as msg;
else 
select concat('This ID (',ID,') is not exist') as msg;
end if;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `view_waste_request_status` (IN `num` VARCHAR(40))   begin
select request_id ID, citizen.name 'Citizen Name', industry.ind_name 'Industry', waste.waste_type 'Waste Type', request_date 'Request Date', schedule 'Date wanted', status 'Status' from waste_request,citizen,industry,waste where waste_request.cit_id=citizen.cit_id and waste_request.ind_id=industry.ind_id and waste_request.waste_id=waste.waste_id and status=num;
end$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `view_waste_req_indus` (IN `num` INT)   begin
select request_id 'ID', citizen.name 'Citizen Name', industry.ind_name 'Industry Name', waste.waste_type 'Waste Type', request_date 'Date', schedule 'Schedule', waste_request.status 'Status' from waste_request,citizen,industry,waste where waste_request.cit_id=citizen.cit_id and waste_request.ind_id=industry.ind_id and waste_request.waste_id=waste.waste_id and waste_request.deleted=0 and industry.ind_id=num;
end$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `waste_between_2_dates` (IN `date1` DATE, IN `date2` DATE)   begin
select request_id ID, citizen.name 'Citizen Name', industry.ind_name 'Industry', waste.waste_type 'Waste Type', request_date 'Request Date', schedule 'Date wanted', status 'Status' from waste_request,citizen,industry,waste where waste_request.cit_id=citizen.cit_id and waste_request.ind_id=industry.ind_id and waste_request.waste_id=waste.waste_id and waste_request.request_date between date1 and date2;
end$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `waste_request_proc` (IN `ID` INT, IN `status_sp` VARCHAR(40), IN `oper` VARCHAR(40))   begin
case 

when oper="update" then 
if exists (select * from waste_request where request_id=ID) then
update waste_request set status=status_sp where request_id=ID;
select 'Updated success' as msg;
else 
select concat('This ID (',ID,') is not exist') as msg;
end if;
...
INSERT INTO `citizen` (`cit_id`, `name`, `tell`, `password`, `email`, `home_number`, `add_no`, `image`, `deleted`) VALUES
(7, 'Mohamednor', '9090', '123', 'mohamednorboorow@gmail.com', '9909', 3, '', 0),
(9, 'moha', '619602548', '12345', 'admin@gmail.com', '9090', 3, '', 0),
...
(1, 'Wasaaradda Naddafadda Deegaanka', 'Mins2002@gmail.so', 1, 0);
...
(8, 3, 2, 9, 10, 0.01, '0000-00-00', 2, 1),
(9, 2, 10, 7, 200, 0.1, '2024-04-26', 1, 0),
(10, 3, 2, 7, 23, 0.05, '2024-04-26', 2, 0);
...
(29, 7, '1449838711.png', 'jkjk', 'progress', '2024-04-15 18:18:46', 0),
(30, 7, '1820166068.png', 'jhjhh', 'progress', '2024-04-15 18:24:26', 0),
(31, 7, '1767294793.png', 'uijjk', 'done', '2024-04-15 19:05:47', 0);
...
(3, 'Saciid', '12345', 'Gov', 'Gov2024.so', 0),
(4, 'Salmo', '1234', 'industry', 'sal@gmail.com', 0);
...
INSERT INTO `waste_request` (`request_id`, `cit_id`, `ind_id`, `waste_id`, `request_date`, `schedule`, `status`, `deleted`) VALUES
(25, 7, 1, 2, '2024-04-15 18:04:20', '2024-04-20 21:04:00', 'done', 0),
(26, 7, 1, 1, '2024-04-15 18:09:50', '2024-04-26 21:09:00', 'Pending', 0),
(27, 7, 2, 1, '2024-04-15 18:10:05', '2024-05-10 21:10:00', 'progress', 0),
(28, 7, 1, 1, '2024-04-15 18:11:51', '2024-04-26 21:11:00', 'Pending', 0),
(29, 7, 2, 3, '2024-04-15 18:12:30', '2024-04-26 21:11:00', 'Pending', 0),
(30, 7, 2, 1, '2024-04-15 18:15:37', '2024-04-26 21:15:00', 'Pending', 0),
(31, 7, 1, 1, '2024-04-15 18:31:25', '2024-04-27 21:31:00', 'Pending', 0),
(35, 7, 1, 1, '2024-04-03 21:00:00', '2024-04-10 00:00:00', 'Pending', 0),
(36, 7, 10, 3, '2024-04-20 09:10:23', '2024-04-21 12:10:00', 'Pending', 0),
(37, 7, 2, 2, '2024-04-20 09:11:32', '2024-04-20 12:12:00', 'Pending', 0),
(38, 9, 2, 2, '2024-04-22 03:26:43', '2024-04-22 07:26:00', 'Pending', 0),
(39, 7, 10, 1, '2024-04-22 03:33:41', '2024-04-23 10:36:00', 'Pending', 0);
...
--
ALTER TABLE `citizen`
  MODIFY `cit_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=116;

--
-- AUTO_INCREMENT for table `goverment_recyle`
--
ALTER TABLE `goverment_recyle`
  MODIFY `gov_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=5;
...
--
ALTER TABLE `recycling`
  MODIFY `recy_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=11;

--
-- AUTO_INCREMENT for table `reporting`
--
ALTER TABLE `reporting`
  MODIFY `rep_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=32;
